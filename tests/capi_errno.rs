//! Tests for ext4rs_last_errno — the POSIX errno companion to
//! ext4rs_last_error. FSKit needs a numeric errno to build NSError
...
// Match the POSIX values from ext4rs::error::errno (macOS).
const ENOENT: i32 = 2;
const EIO: i32 = 5;
...
fn success_clears_errno_to_zero() {
    ...
    let rc = unsafe { ext4rs_stat(fs, path.as_ptr(), &mut attr) };
    assert_eq!(rc, 0);
...
fn null_args_set_einval() {
    let mut attr: ext4rs_attr_t = unsafe { std::mem::zeroed() };
    let rc = unsafe { ext4rs_stat(std::ptr::null_mut(), std::ptr::null(), &mut attr) };
    assert_eq!(rc, -1);
    assert_eq!(ext4rs_last_errno(), EINVAL);
}
...
fn dir_open_on_regular_file_sets_enotdir() {
    let fs = mount();
    let path = CString::new("/test.txt").unwrap();
    let iter = unsafe { ext4rs_dir_open(fs, path.as_ptr()) };
    assert!(iter.is_null());
    assert_eq!(ext4rs_last_errno(), ENOTDIR);
    unsafe { ext4rs_umount(fs) };
}
...
fn errno_is_thread_local_and_persists_until_next_call() {
    ...
    let mut attr: ext4rs_attr_t = unsafe { std::mem::zeroed() };
    unsafe { ext4rs_stat(fs, path.as_ptr(), &mut attr) };
    let e1 = ext4rs_last_errno();
    assert_eq!(e1, ENOENT);
    // Now a success clears errno.
    let path_ok = CString::new("/test.txt").unwrap();
    unsafe { ext4rs_stat(fs, path_ok.as_ptr(), &mut attr) };
    assert_eq!(ext4rs_last_errno(), 0);
    unsafe { ext4rs_umount(fs) };
}
