...
//! This adds assertions on `ext4rs_last_errno` so the Swift side can
...
fn scratch() -> PathBuf {
    ...
    let dst = PathBuf::from(format!(
        "/tmp/ext4rs_capi_unlink_errno_{}_{n}.img",
        std::process::id()
    ));
...
fn unlink_on_success_clears_errno() {
    ...
    let rc = unsafe { ext4rs_unlink(fs_handle, path_c.as_ptr()) };
    assert_eq!(rc, 0);
    assert_eq!(ext4rs_last_errno(), 0, "success must leave errno=0");
...
fn unlink_missing_path_sets_enoent() {
    ...
    let rc = unsafe { ext4rs_unlink(fs_handle, bad.as_ptr()) };
    assert_eq!(rc, -1);
    assert_eq!(ext4rs_last_errno(), 2, "ENOENT expected for missing path");
    unsafe { ext4rs_umount(fs_handle) };
    let _ = fs::remove_file(&img);
}
...
fn unlink_directory_sets_eisdir() {
    ...
    let rc = unsafe { ext4rs_unlink(fs_handle, dir.as_ptr()) };
    assert_eq!(rc, -1);
    assert_eq!(ext4rs_last_errno(), 21, "expected EISDIR for unlink-on-dir");
    unsafe { ext4rs_umount(fs_handle) };
    let _ = fs::remove_file(&img);
}
...
fn unlink_null_fs_sets_einval() {
    ...
    let rc = unsafe { ext4rs_unlink(std::ptr::null_mut(), path.as_ptr()) };
    assert_eq!(rc, -1);
...
fn double_unlink_second_call_is_enoent() {
    ...
    let rc1 = unsafe { ext4rs_unlink(fs_handle, path.as_ptr()) };
    assert_eq!(rc1, 0, "first unlink");
    let rc2 = unsafe { ext4rs_unlink(fs_handle, path.as_ptr()) };
    assert_eq!(rc2, -1, "second unlink MUST fail");
    assert_eq!(ext4rs_last_errno(), 2, "second unlink errno=ENOENT");
...
fn unlink_root_fails() {
    ...
    let rc = unsafe { ext4rs_unlink(fs_handle, root.as_ptr()) };
...
fn unlink_null_path_sets_einval() {
    ...
    let fs_handle = unsafe { ext4rs_mount_rw(img_c.as_ptr()) };
    assert!(!fs_handle.is_null());
    let rc = unsafe { ext4rs_unlink(fs_handle, std::ptr::null()) };
    assert_eq!(rc, -1);
    assert_eq!(ext4rs_last_errno(), 22); // EINVAL
    unsafe { ext4rs_umount(fs_handle) };
    let _ = fs::remove_file(&img);
