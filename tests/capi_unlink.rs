//! C-ABI tests for `ext4rs_unlink`. Each test makes its own scratch
...
fn scratch_image() -> PathBuf {
    ...
    let dst = PathBuf::from(format!(
        "/tmp/ext4rs_capi_unlink_{}_{n}.img",
        std::process::id()
    ));
...
fn unlink_null_inputs_do_not_crash() {
    ...
    let path_c = CString::new("/test.txt").unwrap();
    let rc = unsafe { ext4rs_unlink(std::ptr::null_mut(), path_c.as_ptr()) };
    assert_eq!(rc, -1);
    let rc = unsafe { ext4rs_unlink(fs, std::ptr::null()) };
    assert_eq!(rc, -1);
