...
fn mount_or_skip() -> Option<*mut ext4rs_fs_t> {
    if !Path::new(IMAGE).exists() { return None; }
...
fn readlink_on_basic_link_returns_expected_target() {
    ...
    if !matches!(attr.file_type, ext4rs_file_type_t::Symlink) {
        eprintln!("skip: /link.txt exists but isn't a symlink (file_type={:?})", attr.file_type as u32);
        unsafe { ext4rs_umount(fs) };
        return;
    }
    ...
    let mut buf = [0u8; 256];
    let rc = unsafe {
        ext4rs_readlink(fs, p.as_ptr(), buf.as_mut_ptr() as *mut i8, buf.len())
    };
