...
fn mount_ro(bytes: &Vec<u8>) -> *mut ext4rs_fs_t {
...
fn create_on_ro_mount_refused_with_erofs() {
    ...
    let ino = unsafe { ext4rs_create(fs_h, p.as_ptr(), 0o644) };
    assert_eq!(ino, 0, "create on RO mount must fail");
    assert_eq!(ext4rs_last_errno(), 30, "EROFS (30) expected on RO mount");
...
fn mkdir_on_ro_mount_refused() {
    ...
    let ino = unsafe { ext4rs_mkdir(fs_h, p.as_ptr(), 0o755) };
    assert_eq!(ino, 0);
...
fn rmdir_on_ro_mount_refused() {
    ...
    let rc = unsafe { ext4rs_rmdir(fs_h, p.as_ptr()) };
    assert_eq!(rc, -1);
    assert_ne!(ext4rs_last_errno(), 0);
    unsafe { ext4rs_umount(fs_h) };
}
