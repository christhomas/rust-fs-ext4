//! C-ABI tests for `ext4rs_rename`. Covers intra-dir rename,
...
fn scratch_image() -> PathBuf {
    ...
    let dst = PathBuf::from(format!(
        "/tmp/ext4rs_capi_rename_{}_{n}.img",
        std::process::id()
    ));
...
fn stat_ino(fs: *mut ext4rs_fs_t, path: &str) -> u32 {
    let p = CString::new(path).unwrap();
    let mut attr: ext4rs_attr_t = unsafe { std::mem::zeroed() };
...
fn rename(fs: *mut ext4rs_fs_t, src: &str, dst: &str) -> i32 {
    let s = CString::new(src).unwrap();
    let d = CString::new(dst).unwrap();
    unsafe { ext4rs_rename(fs, s.as_ptr(), d.as_ptr()) }
}
...
fn cross_parent_directory_move_updates_dotdot() {
    ...
    let d_ino = unsafe { ext4rs_mkdir(fs, mdir.as_ptr(), 0o755) };
    assert!(d_ino > 0, "mkdir: {}", last_err_str());
    let c_ino = unsafe { ext4rs_create(fs, child.as_ptr(), 0o644) };
