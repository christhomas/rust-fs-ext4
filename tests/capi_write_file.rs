//! C-ABI tests for `ext4rs_write_file` — the "save-as" path: replace
//! an existing file's body with new bytes. Pairs naturally with
//! `ext4rs_create` (create + write).
...
fn scratch_image() -> PathBuf {
    ...
    let dst = PathBuf::from(format!(
        "/tmp/ext4rs_capi_write_file_{}_{n}.img",
        std::process::id()
    ));
...
fn read_all(fs: *mut ext4rs_fs_t, path: &str, stat_size: u64) -> Vec<u8> {
...
fn write_bytes(fs: *mut ext4rs_fs_t, path: &str, data: &[u8]) -> i64 {
    let p = CString::new(path).unwrap();
    unsafe {
        ext4rs_write_file(
            fs,
