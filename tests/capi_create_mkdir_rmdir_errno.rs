...
//! `ext4rs_last_errno`. POSIX codes FSKit needs to surface:
//!
...
fn scratch(label: &str) -> PathBuf {
    ...
    let dst = PathBuf::from(format!(
        "/tmp/ext4rs_cmr_errno_{label}_{}_{n}.img",
        std::process::id()
    ));
...
fn create_on_existing_file_sets_eexist() {
    ...
    let ino = unsafe { ext4rs_create(fs_h, existing.as_ptr(), 0o644) };
    assert_eq!(ino, 0, "create on existing must fail");
    assert_eq!(ext4rs_last_errno(), 17, "EEXIST");
...
fn mkdir_on_existing_dir_sets_eexist() {
    ...
    let ino = unsafe { ext4rs_mkdir(fs_h, existing.as_ptr(), 0o755) };
...
fn rmdir_on_nonempty_dir_sets_enotempty() {
    ...
    let rc = unsafe { ext4rs_rmdir(fs_h, dir.as_ptr()) };
    assert_eq!(rc, -1);
    assert_eq!(ext4rs_last_errno(), 66, "ENOTEMPTY (macOS)");
...
fn rmdir_on_regular_file_sets_enotdir() {
    ...
    let rc = unsafe { ext4rs_rmdir(fs_h, file.as_ptr()) };
    assert_eq!(rc, -1);
...
fn create_null_args_set_einval() {
    let ino = unsafe { ext4rs_create(std::ptr::null_mut(), std::ptr::null(), 0o644) };
    assert_eq!(ino, 0);
...
fn mkdir_null_args_set_einval() {
    let ino = unsafe { ext4rs_mkdir(std::ptr::null_mut(), std::ptr::null(), 0o755) };
    assert_eq!(ino, 0);
    assert_eq!(ext4rs_last_errno(), 22);
}
...
fn rmdir_null_args_set_einval() {
    let rc = unsafe { ext4rs_rmdir(std::ptr::null_mut(), std::ptr::null()) };
    assert_eq!(rc, -1);
    assert_eq!(ext4rs_last_errno(), 22);
}
