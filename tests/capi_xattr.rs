...
fn listxattr_probe_returns_required_size() {
    ...
    let required = unsafe {
        ext4rs_listxattr(fs, path.as_ptr(), std::ptr::null_mut(), 0)
    };
...
fn listxattr_writes_names_nul_separated() {
    ...
    assert!(names.contains(&"user.color".into()), "missing user.color: {names:?}");
    assert!(
...
fn listxattr_plain_file_has_no_user_xattrs() {
    ...
    assert!(user.is_empty(), "plain.txt should have no user.* xattrs: {user:?}");
...
fn getxattr_zero_length_value_returns_zero_not_minus_one() {
    ...
    let sz = unsafe {
        ext4rs_getxattr(fs, path.as_ptr(), name.as_ptr(), std::ptr::null_mut(), 0)
    };
    assert_eq!(sz, 0, "system.data should exist with 0-byte value, got {sz}");
